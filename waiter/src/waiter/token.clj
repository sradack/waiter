...
(defn handle-token-request
  ...
  (case request-method
    :delete (let [{:keys [token]} (sd/retrieve-token-from-service-description-or-hostname headers headers waiter-hostname)
    ...
    :get (let [request-params (:query-params (ring-params/params-request req))
             ...
             (if (and service-description-template (not-empty service-description-template))
               ...
               (do
                 (throw (ex-info (str "Couldn't find token " token) 
                                 {:status 404 :token token})))))) 
    :post (let [request-params (:query-params (ring-params/params-request req))
                authenticated-user (get req :authorization/user)
